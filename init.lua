...
local function climate(x, z, y, n_terr, n_terr2)
	...
	--blending
	local blend = (((n_terr2 + n_terr)/2) * 5)
	...
	for z = minp.z, maxp.z do
        ...
        local mup = (1 - xtgrad) + (-1 * xtgrad)
				...
				or den_sedi > t_sedi then
					...
					local shelfsl = (5*y) - (CONOI*2) --sets slope
					...
					-----------------------------
					--caldera island
					local calir = (40 + (20 * n_terr) + (20 * n_terr2)) * (1 - (y/(25 + n_terr2)))
					...
					local channel = (100 +(n_terr2*15))*math.cos(xab/(42 + (n_terr * (3*(1-xtgrad)))))
					local w = (15 + (n_terr*3)) * (1 + (y/(10 + (n_terr2*2) - (5*xtgrad))))
