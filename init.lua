...
local function ore(b_stone, void, y, data, vi, ystrata, TSTRA, OREID, n_cave, n_cave2, n_strata, n_terr, n_base)
	...
	local ore_n1 = math.abs(n_cave2 - (n_strata * 0.02) + (n_terr * 0.045))  --abundant
	local ore_n2 = math.abs(n_cave2 - (n_strata * 0.005) + (n_terr * 0.0055))  --rare
	local ore_t1 = 0.95 - (n_terr * 0.01) - (n_base * 0.01)  --abundant
	local ore_t2 = 0.98 - (n_terr * 0.0025) - (n_base * 0.001) --rare
	local orehmin_c = -5000 + (n_strata * 1000) --min height for coal (a shallow ore)
	local orehmax_d = -150 + (n_cave * 100)   --diamonds are deep
	local orehmax_m = -250 + (n_cave * 200)   --mese is deep
        ...
        local sedi = 		false --subsurface dirt, sand etc
		        ...
		        --Are There Caves? or Canyons
						...
						if y < 80 then
							...
							local cav_lim1 = PCAV.MAXFIS - (ab_stra*10)
							local cav_lim2 = PCAV.MAXCAV - (ab_stra*10)
							-- so that depth leads to bigger caves.
			        ...
			        local cav_t1 = (PCAV.BCAVTF + (ab_terr*0.01) + (ab_base*0.01))/ybig
			        ...
			        if y < cav_lim2
			        and (n_cave >= cav_t2 or n_cave2 >= cav_t2 or n_canyon >= cav_t2) then
			          nocave = false
			        end
							...
							--(let's not even pretend they are rivers... isn't happening!)
							local ab_can = math.abs(n_canyon)
							--lower the canyon floor downslope..start very high in mounts... then drop
							local canfloor = 100*math.exp(-0.00022*xab)		--(-0.0008*xab) + 20
							nocanyon = false
...
function spawnplayer(player)
	...
	inventory:add_item("main", "default:torch")
	inventory:add_item("main", "default:torch")
	inventory:add_item("main", "default:torch")
	inventory:add_item("main", "default:torch")
	inventory:add_item("main", "default:torch")
	...
	inventory:add_item("main", "default:torch")
	inventory:add_item("main", "default:torch")
	inventory:add_item("main", "default:ladder")
	inventory:add_item("main", "default:ladder")
	inventory:add_item("main", "default:ladder")
	inventory:add_item("main", "default:ladder")
	inventory:add_item("main", "default:ladder")
